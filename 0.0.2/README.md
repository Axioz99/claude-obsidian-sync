    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node D:/path/to/claude-obsidian-sync/dist/hooks/hook-handler.js",
            "async": true
          }
        ]
      }
...
**注意**：
- 将路径替换为你实际的项目 `dist/hooks/hook-handler.js` 路径
- 使用 `node` 直接运行编译后的 JS 文件，性能更好
- `async: true` 让 PostToolUse hook 异步执行，不阻塞 Claude Code
- `matcher` 使用正则匹配需要跟踪的工具
