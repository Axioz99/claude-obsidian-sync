...
async function handleToolUse(
  ...
  const { tool_name, tool_input, tool_response, hook_event_name } = input;
...
async function main(): Promise<void> {
  try {
    ...
    // 调试：记录原始输入
    await log('DEBUG', 'Raw input received', { length: inputStr.length, preview: inputStr.substring(0, 500) });

    const input: HookInput = JSON.parse(inputStr);
    await log('DEBUG', 'Parsed hook input', {
