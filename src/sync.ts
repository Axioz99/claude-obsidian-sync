...
export class ObsidianSync {
  ...
  private getObservationsPath(): string {
    return path.join(this.config.vaultPath, this.config.baseFolder, '观察');
  }
  ...
  private getSummariesPath(): string {
    return path.join(this.config.vaultPath, this.config.baseFolder, '摘要');
  }
  ...
  async syncObservations(
    ...
    for (const item of items) {
      const result = await this.syncObservation(item.observation, item.metadata);
      results.push(result);
    }
    return results;
  }
  ...
  async syncSummaries(
    ...
    const results: SyncResult[] = [];
    for (const item of items) {
      const result = await this.syncSummary(item.summary, item.metadata);
      results.push(result);
    }
    return results;
  }
